...
<?php
	...
	{ $list_exp[$a] = $a; }
	if($expire_days <= 30)
		$expire_days2 = $expire_days;
	$list_expire_days = array("0" => "--");
	for ($a=1; $a <= 30; $a++)
	{ 
		if(!empty($expire_days2)){
			$expire_days = $expire_days2;
		}else{
			if($a == 14) 
				$expire_days = '14';
		}
...
*/
	...
	if ((($_SESSION["userlevel"] == "superadmin") || ($_SESSION["userlevel"] == "admin")) && $status=="4") {
		echo "<div class='cell_val'><input type=\"button\" value=\"Cancel\" id=\"btncancel\" onClick=\"window.location='".base_url()."admin/jobpost/view/".$job_id."'\"/></div>";	
		echo "<div class='cell_val'>".form_submit("save", "Save")."</div>";
		//echo "<div class='cell_val'><input type=\"button\" value=\"Approve, Activate\" id=\"btnaktif\" /></div>";
	} else if ((($_SESSION["userlevel"] == "superadmin") || ($_SESSION["userlevel"] == "admin")) && $status=="1") {
		echo "<div class='cell_val'><input type=\"button\" value=\"Cancel\" id=\"btncancel\" onClick=\"window.location='".base_url()."admin/jobpost/view/".$job_id."'\"/></div>";	
		echo "<div class='cell_val'>".form_submit("save", "Save")."</div>";
		//echo "<div class='cell_val'><input type=\"button\" value=\"Approve, Activate\" id=\"btnaktif\" /></div>";		
	} else {
		$jshapus = 'onClick="document.getElementById('."'aksi'".').value='."'ashapus'".';document.forms[0].submit()";';
		$jsnext = 'onClick="document.getElementById('."'aksi'".').value='."'asnext'".';document.forms[0].submit()";';	
		echo "<div class='cell_val'><input type=\"button\" value=\"Discard\" id=\"btnhapus\" $jshapus /></div>";	
		echo "<div class='cell_val'>".form_submit("edit", "Save as Draft")."</div>";
		echo "<div class='cell_val'><input type=\"button\" value=\"Next\" id=\"btnext\" $jsnext /></div>";
	}
	echo "</div></div>";	
