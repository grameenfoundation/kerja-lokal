...
class Jobposter extends CI_Controller {
	...
	function add()
	{
		...
		//{
			...
			$data['form_action'] = base_url()."user/jobposter/add";

			$this->template->write("title", $title);
			$this->template->write_view("content", "user/jobposter_edit", $data, TRUE);
		//}
		//else
		...
		if ($this->Madmin->check_access($_SESSION["userlevel"], $useraccess) == "1")
		{
			if (sizeof($_POST) > 0)
			{
				...
				if (($search["status"] != "_") || ($search["username"] != "_") || ($search["phone"] != "_") || ($search["mobile"] != "_") || ($search["userlevel"] != "_") || ($search["email"] != "_") || ($search["position"] != "_") || ($search["date_add"] != "_")) 
					...
					redirect(base_url()."admin/jobposter/manage/$page/$order/".urlencode($search["status"])."/".urlencode($search["username"])."/".urlencode($search["phone"])."/".urlencode($search["mobile"])."/".urlencode($search["userlevel"])."/".urlencode($search["email"])."/".urlencode($search["position"])."/".urlencode($search["date_add"]));
			}
