...
class Jobsent_report_njfu extends CI_Controller {
	...
	function manage($page=1, $order="a_jobsend_id", $default_row=20, $date_send1="", $date_send2="", $status="", $jobseeker_name="", $jobcat_id="", $jobcat_id2="", $comp_id="", $mdn="", $sms_status="",$type="", $job_title="", $seekerkodya="", $jobkodya="", $rel_id="", $no_njfu=""){ //$loc_id="", $loc_title="", $company_id="", 
		...
		{			
			...
			$jobsent = CORE_URL."get_all_jobsent_njfu.php?tx_id=$tx_id&ndata=$ndata&page=$page&order=$orderby&ascdesc=$ascdesc&id=".$_SESSION["userid"].'&'.$search_uri;				
			...
			$data['filter']['form_submit'] = base_url()."admin/jobsent_report_njfu/manage/1/d_jobsend_id";
			//echo "<pre>awal"; print_r($data['search']); echo "</pre><hr>";
			$data['search_form'] = $this->load->view('admin/jobsent_report_njfu_searchform_adv',$data['filter'],TRUE);
	...
	//function save_csv($order="a_jobseeker_name", $status="", $date_send1="", $sent_time1="", $jobseeker_name="", $job_kotamadya="", $date_send2="")
	{
		...
		if ($this->Madmin->check_access($_SESSION["userlevel"], $useraccess) == "1")
		{
			...
			header("Content-Disposition: attachment; filename=jobsent_no_job.csv");
			...
			//	{ if ($search[$key] != "_") $var .= "&$key=".urlencode($val); }
				...
				if (($_SESSION["userlevel"] == "company") || ($_SESSION["userlevel"] == "jobposter")){
					$json = CORE_URL."get_all_jobsent_njfu.php?tx_id=$tx_id$var&ndata=$ndata&order=$orderby&ascdesc=$ascdesc&id=".$_SESSION["userid"].'&'.$search_uri;
					//echo"1<pre>";print_r($json);echo"</pre>";exit();
				}else{
					$json = CORE_URL."get_all_jobsent_njfu.php?tx_id=$tx_id&ndata=$ndata&order=$orderby&ascdesc=$ascdesc".'&'.$search_uri;
					//echo"2<pre>";print_r($json);echo"</pre>";exit();
				}
			}
			else
			{
				//echo "jajal"; page=$page&
				if (($_SESSION["userlevel"] == "company") || ($_SESSION["userlevel"] == "jobposter"))
			...
			//print_r($data);
			$output = "jobsend_id, send_date, sent_time, subscription_id, subscription_category, subscriber_id, mdn, jobseeker_name, jobseeker_kodya, job_id, job_title, job_category, distance, status, send_date2, sent_time2 \n";
			foreach($data["results"] as $job)
			{									
				$subtype=($job["mentor_id"]=='0') ? "APP" : "SMS"; //$jobcat["mentor_id"].
