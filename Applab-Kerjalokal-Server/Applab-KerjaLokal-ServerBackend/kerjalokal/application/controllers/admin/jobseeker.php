...
class Jobseeker extends CI_Controller {
	...
	function resetpass($subscriber_id=0,$newpass="") {
		...
		if ($this->Madmin->check_access($_SESSION["userlevel"], $useraccess) == "1")
		{
			...
			for($ctr=0;$ctr<4;$ctr++) {
				$newpass = $newpass . $syms[rand(0,strlen($syms)-1)];
			}
			
			// change the password
			$update_url = CORE_URL."update_subscriber.php?subscriber_id=".$subscriber_id."&pinweb=".$newpass;
			$this->Madmin->get_data($update_url);
			
			
			$data['newpassword'] = $newpass;
			
			$this->template->write("title", $title);
			$this->template->write_view("content", "admin/jobseeker_reset", $data, TRUE);
		}
		else
	...
	//function search_form_adv($status=1, $loc_title="", $gender="", $education="", $jobseeker_name="", $mdn="", $mentor="", $birthday="")
	{
		...
		*/
		...
		$sql1 = CORE_URL."get_education_by_status.php?key=status&value=1";
