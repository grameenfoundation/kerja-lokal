...
class Jobposter extends CI_Controller {
		...
		if ($this->Madmin->check_access($_SESSION["userlevel"], $useraccess) == "1")
		{
			...
			if (sizeof($_POST) > 0)
			{
				...
				if ($this->form_validation->run() == TRUE)
				{
					...
					$var = substr($var,1);
					$json = CORE_URL."add_jobposter.php?tx_id=$tx_id&$var";
					//die($json);
			...
			$data['form_action'] = base_url()."admin/jobposter/add";

			$this->template->write("title", $title);
			$this->template->write_view("content", "admin/jobposter_edit", $data, TRUE);
		}
		else
	...
	function manage($page = 1, $order="a_username", $status="", $username="", $userlevel="", $email="", $date_add="")
	{
		$useraccess = array("superadmin", "admin", "company");
		...
		if ($this->Madmin->check_access($_SESSION["userlevel"], $useraccess) == "1")
		{
			if (sizeof($_POST) > 0)
			{
				...
				if (($search["status"] != "_") || ($search["username"] != "_") || ($search["userlevel"] != "_") || ($search["email"] != "_") || ($search["date_add"] != "_")) 

					redirect(base_url()."admin/jobposter/manage/$page/$order/".urlencode($search["status"])."/".urlencode($search["username"])."/".urlencode($search["userlevel"])."/".urlencode($search["email"])."/".urlencode($search["date_add"]));
			}
			...
			$data["search_link"] = base_url()."admin/jobposter/search_form/".$search["status"]."/";
	...
	function edit($id=0)
	{
		...
		if ($this->Madmin->check_access($_SESSION["userlevel"], $useraccess) == "1")
		{
			...
			$data['form_action'] = base_url()."admin/jobposter/edit/$id";
