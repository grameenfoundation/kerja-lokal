<?php
	...
	$date_active_from = isset($date_active_from) ? ($date_active_from == '' ? '' : str_clean($date_active_from)) : '';
	//echo $date_active_from."<hr>";
	$date_active_to = isset($date_active_to) ? ($date_active_to == "null" ? '' : str_clean($date_active_to)) : '';	
	...
	if (!empty($date_active_from)) $search .= "AND $t_rel_subscriber_cat.date_add BETWEEN '$date_active_from 00:00:01' AND '$date_active_to 00:00:01' ";	
	...
	$sql .= "SELECT $t_rel_subscriber_cat.rel_id AS rel_id, $t_rel_subscriber_cat.status AS status, 
		...
		$t_subscribers.mdn AS mdn, $t_subscribers.subscriber_id AS subscriber_id, $t_subscribers.name AS subscriber_name, $t_subscribers.loc_id AS subscriber_loc_id,
		$t_subscribers.mentor_id, $t_location.name AS kelurahan_name
		FROM $t_subscribers 
	...
	while($row = mysql_fetch_assoc($sql))
	{
		...
		$kecamatan_id = substr($val["subscriber_loc_id"],5);
		...
		/*
		$sql2 = "SELECT *,
					SUM(IF(description = 'No job for you', 1,0)) AS `n_njfu`,
					SUM(IF(description != 'No job for you', 1,0)) AS `n_job`,
					COUNT(description) AS `n_all_job`
					FROM jobs
					inner join jobs_send on jobs.`job_id` = `jobs_send`.`job_id`
					where `jobs_send`.`subscriber_id`='$subscriber_id' GROUP BY `jobs_send`.`rel_id`
				";
		*/
		$sql2 =	"SELECT *,
				...
				";	
				//WHERE `rel_id`='$rel_id' and status='2' GROUP BY `jobs_send`.`rel_id`	DEFAULT			
		//echo $sql2."<hr>";		
		$sql2 = mysql_query($sql2) OR die(output(mysql_error()));
		$row2 = mysql_fetch_assoc($sql2);
