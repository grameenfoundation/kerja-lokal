<?php
	...
	$jbposter_id = isset($_GET["jbposter_id"]) ? ($_GET["jbposter_id"] == "null" ? '' : str_clean($_GET["jbposter_id"])) : '';
	...
	{ 
		...
		case "loc_name" : $order = "$t_location.name"; break;
		case "company_name" : $order = "$t_jobs.comp_name"; break;
		case "date_add" : $order = "$t_jobs.date_add"; break;
		case "approved_id" : $order = "$t_jobs.approved_id"; break;
		case "approved_date" : $order = "$t_jobs.approved_date"; break;
	...
	if (!empty($status) && ($status != "")) $search .= " AND $t_status.status_id = $status ";	
	if (!empty($submit_date_from)) $search .= "AND $t_jobs.date_add BETWEEN '$submit_date_from 00:00:01'  AND '$submit_date_to 23:59:59' ";
	...
	if (!empty($jbposter_id)) $search .= "AND $t_jobs.jobposter_id = $jbposter_id ";
	if (!empty($approver_id)) $search .= "AND $t_jobs.approver_id = $approver_id ";
	if (!empty($approved_date_to)) $search .= "AND $t_jobs.approved_date BETWEEN '$approved_date_from' AND '$approved_date_to' ";
	...
	{		
		...
		if ($r["userlevel"] == "company")
		{
			...
			$sql = "SELECT *, $t_jobs.title as job_title, $t_jobs.description AS job_desc, $t_jobs.status AS job_status, 
				...
				$t_jobs.comp_id AS company_id $t_jobs.comp_id AS company_id, pany_name, $t_companies.description AS comp_desc, $t_jobs.date_add AS date_add 
				FROM ($t_jobs INNER JOIN $t_job_posters ON $t_jobs.jobposter_id=$t_job_posters.jobposter_id)
				INNER JOIN $t_status ON $t_jobs.status = $t_status.status_id
				INNER JOIN $t_location ON $t_jobs.loc_id = $t_location.loc_id
				INNER JOIN $t_jobs_category ON $t_jobs_category.jobcat_id = $t_jobs.jobcat_id
