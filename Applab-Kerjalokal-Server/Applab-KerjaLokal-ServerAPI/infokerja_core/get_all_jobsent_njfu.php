...
//	$jobkodya = isset($_GET["jobkodya"]) ? ($_GET["jobkodya"] == "_" ? '' : str_clean($_GET["jobkodya"])) : '';
	...
	if (!empty($jobcat_id2)) $search .= "AND $t_rel_subscriber_cat.jobcat_id = $jobcat_id2 ";	
	...
	$sql = "SELECT $t_jobs_send.jobsend_id, DATE($t_jobs_send.date_send) AS date_send1, DATE_FORMAT($t_jobs_send.date_send,'%h:%i:%s %p') AS sent_time1,
		$t_rel_subscriber_cat.rel_id, $t_rel_subscriber_cat.jobcat_key,$t_subscribers.mentor_id AS mentor_id, $t_subscribers.subscriber_id,
		...
		WHERE $t_log_sms.title='sms' and jobs.description = 'No job for you'
	";
