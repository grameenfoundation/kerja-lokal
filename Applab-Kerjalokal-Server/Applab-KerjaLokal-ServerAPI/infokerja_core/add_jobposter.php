...
{ 
	...
	if ($key == "userlevel")
		...
		{
			$sql = "SELECT MAX(jobposter_id) as jobposter_id FROM $t_job_posters WHERE userlevel = 'superadmin' OR userlevel = 'admin'";
...
{
	$sql = "INSERT INTO $t_job_posters ($var_k) VALUES ($var_v)";
	//die($sql);
	$r = mysql_query($sql) OR die(output(mysql_error()));
