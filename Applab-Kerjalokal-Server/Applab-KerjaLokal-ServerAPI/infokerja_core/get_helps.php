<?php
	...
	//$order = isset($_GET["order"]) ? str_clean(strtoupper($_GET["order"])) : "help_id";
	if($_GET["order"] == "help_id" OR $_GET["order"] == "status") $d = ""; else $d = $t_help.".";
	$order = isset($_GET["order"]) ? $d.str_clean($_GET["order"]) : "help_id";
	
	
	$ascdesc = isset($_GET["ascdesc"]) ? str_clean(strtoupper($_GET["ascdesc"])) : "ASC";
	$callback = isset($_GET['callback']);
	
	$help_title = isset($_GET["help_title"]) ? ($_GET["help_title"] == "_" ? "" : str_clean($_GET["help_title"])) : "";
	...
	{ 
		...
		case "help_title" : $order = "help_title"; break;
	...
	if ($status != 0) $sql .= "WHERE $t_help.status='$status'";
	$sql = mysql_query($sql) OR die(output(mysql_error()));
	$arr["totaldata"] = mysql_num_rows($sql);
	$arr['ndata'] = $ndata == "0" ? $arr["totaldata"] : $ndata;
	
	$sql = "SELECT *, $t_status.title AS status_title FROM $t_status INNER JOIN $t_help ON $t_help.status = $t_status.status_id ";
	if ($status != 0) $sql .= "WHERE $t_help.status='$status'";
